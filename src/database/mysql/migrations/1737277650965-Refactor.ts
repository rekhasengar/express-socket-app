...
export class Refactor1737277650965 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`messages\` (\`key\` int NOT NULL AUTO_INCREMENT, \`id\` varchar(36) NOT NULL, \`createAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updateAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted\` tinyint NOT NULL DEFAULT 0, \`senderKey\` int NOT NULL, \`message\` text NULL, \`conversationKey\` int NOT NULL, UNIQUE INDEX \`IDX_18325f38ae6de43878487eff98\` (\`id\`), PRIMARY KEY (\`key\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `ALTER TABLE \`socket\` ADD CONSTRAINT \`FK_78df0073f4afedb6c8f5871bd0c\` FOREIGN KEY (\`userKey\`) REFERENCES \`users\`(\`key\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`messageStatuses\` ADD CONSTRAINT \`FK_ba2f4cb3aa4046a05a01129040c\` FOREIGN KEY (\`messageKey\`) REFERENCES \`messages\`(\`key\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`messageStatuses\` ADD CONSTRAINT \`FK_1b73de01772f50076bcbdbf9d71\` FOREIGN KEY (\`userKey\`) REFERENCES \`users\`(\`key\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`conversationMembers\` ADD CONSTRAINT \`FK_009fd69d83bb58affa899e7858a\` FOREIGN KEY (\`userKey\`) REFERENCES \`users\`(\`key\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`conversationMembers\` ADD CONSTRAINT \`FK_d191b06e5c87ddd516f4e9a25fb\` FOREIGN KEY (\`conversationKey\`) REFERENCES \`conversations\`(\`key\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`conversationMembers\` ADD CONSTRAINT \`FK_ed76ade811b52288c9063c5ad17\` FOREIGN KEY (\`roleKey\`) REFERENCES \`roles\`(\`key\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`conversations\` ADD CONSTRAINT \`FK_27f7953ef926aca77198161c5a5\` FOREIGN KEY (\`createdByKey\`) REFERENCES \`users\`(\`key\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`messages\` ADD CONSTRAINT \`FK_21de9e48c6e40d8e3e342ec471b\` FOREIGN KEY (\`senderKey\`) REFERENCES \`users\`(\`key\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
      `ALTER TABLE \`messages\` ADD CONSTRAINT \`FK_dca53a2adcf9567ac21c665e522\` FOREIGN KEY (\`conversationKey\`) REFERENCES \`conversations\`(\`key\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }
